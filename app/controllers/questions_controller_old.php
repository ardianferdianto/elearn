...
class QuestionsController extends AppController {
	...
	function add_single($kelas=null,$mapel=null,$level=null,$tipe=null) {
		...
		if (!empty($this->data)) {
			...
			if ($this->Question->saveAll($this->data)) {
				...
				//save questionsid
				$this->Session->setFlash('Pertanyaan berhasil ditambahkan','flash_success');
				...
				$this->Session->setFlash('Pertanyaan gagal disimpan, silahkan ulangi!','flash_erorr');
	...
	function proses($kelas=null,$mapel=null,$level=null,$tipe=null){
	...
	function quizz($kelas=null,$mapel=null,$level=null,$tipe=null,$userid = null){
		...
		//find time
		if ($level == 1) {
			$timeset= $configGandaMudah;
		}elseif ($level ==2) {
			$timeset = $configGandaSedang;
		}elseif($level == 3) {
			$timeset = $configGandaSulit;
		}
    ...
    function import_question($kelas=null,$mapel=null,$level=null,$tipe=null){
		...
		for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
		    ...
		    for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
		        if($i == 1) {
		            ...
		            //column of data
		            $row_data[$headings[9]] = $tipe;
		            $row_data[$headings[10]] = $kelas;
		            $row_data[$headings[11]] = $mapel;
		            $row_data[$headings[12]] = $level;
		...
		if($this->Question->saveAll($xls_data, array('validate'=>false))) {
			$this->Session->setFlash('Berhasil mengimpor '. count($xls_data) .' data.','flash_success');

		    $this->redirect(array('controller'=>'questions','action'=>'proses/'.$kelas.'/'.$mapel.'/'.$level.'/'.$tipe));
		    //$this->redirect(array('action'=>'index'));
		} else {
			$this->Session->setFlash('Error, tidak berhasil mengimpor data, silahkan ulangi!','flash_erorr');
