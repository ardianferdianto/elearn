 ...
 */
	...
	var $appLicationName = 'Anjungan Sains';
/**
 ...
 */
	...
	//URL SITE
	var $urlDefault ='http://localhost/anjungan-sains/';
/**
 * beforeFilter
 *
 * @return void
 */
    function beforeFilter() {
		...
		$appLicationName = 'Anjungan Sains';
		$this->set('appLicationName',$appLicationName);
		$urlSite = 'http://localhost/anjungan-sains/';
 ...
 */
    function index() {
		...
		$appLicationName = 'Anjungan Sains';
        $this->pageTitle = __('Welcome: Selamat Datang di Instalasi '.$appLicationName, true);
	...
	function __validateConection()
	{
		...
		$conn = @fsockopen("www.taufiqridha.com", 80, $errno, $errstr, 5);
		if (!$conn)
		{
	...
	function __validateBrowser(){
		...
		$browser = new Browser();
		if( $browser->getBrowser() == Browser::BROWSER_CHROME && $browser->getVersion() >= 17 ) {
			$results['message'] = $this->appLicationName.' dan instalasi '. $this->appLicationName.' berjalan sempurna di Chrome 17.0 atau lebih.';
			$results['status'] = 1;
		}else{	
			$results['message'] = $this->appLicationName.' dan instalasi '. $this->appLicationName.' membutuhkan Chrome versi 17 atau lebih, silahkan install terlebih dahulu';
			$results['status'] = 0;
		}
		return $results;
	}
	...
	function data_sekolah() {
        ...
        if (!empty($this->data)) {
			...
			if ($this->Data->validates()) {
				...
				// Home call details
				$home_url_site = 'http://taufiqridha.com';
				$home_url_port = 80;
				$home_url_iono = '/iono/remote.php';
				$user_defined_string = '3a4bd77f4444';
	...
	function runDbSims(){
		if (isset($this->params['named']['run'])) {
            ...
            if(!$db->isConnected()) {
                ...
                $this->__executeSQLScript($db, CONFIGS.'sql'.DS.'anjungan.sql');
				//$this->__saveInifile();
 ...
 */
    function finish() {
		...
		if (!$this->Session->check('Install.startInstall')) {
			...
			$this->Session->setFlash(__('Selamat Anjungan Sains sudah terinstall.', true));
 ...
 */
    function __executeSQLScript($db, $fileName) {
        ...
        //on quantum mylearn --endQuery added
