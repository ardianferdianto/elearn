<div class="quizzPoints index">
<h2><?php __('QuizzPoints');?></h2>
<p>
<?php
echo $paginator->counter(array(
'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
));
?></p>
<table cellpadding="0" cellspacing="0">
<tr>
	<th><?php echo $paginator->sort('id');?></th>
	<th><?php echo $paginator->sort('siswa');?></th>
	<th><?php echo $paginator->sort('points');?></th>
	<th><?php echo $paginator->sort('details');?></th>
	<th><?php echo $paginator->sort('created');?></th>
	<th><?php echo $paginator->sort('modified');?></th>
	<th><?php echo $paginator->sort('quizz_id');?></th>
	<th class="actions"><?php __('Actions');?></th>
</tr>
<?php
$i = 0;
foreach ($quizzPoints as $quizzPoint):
	$class = null;
	if ($i++ % 2 == 0) {
		$class = ' class="altrow"';
	}
?>
	<tr<?php echo $class;?>>
		<td>
			<?php echo $quizzPoint['QuizzPoint']['id']; ?>
		</td>
		<td>
			<?php echo $quizzPoint['QuizzPoint']['siswa']; ?>
		</td>
		<td>
			<?php echo $quizzPoint['QuizzPoint']['points']; ?>
		</td>
		<td>
			<?php echo $quizzPoint['QuizzPoint']['details']; ?>
		</td>
		<td>
			<?php echo $quizzPoint['QuizzPoint']['created']; ?>
		</td>
		<td>
			<?php echo $quizzPoint['QuizzPoint']['modified']; ?>
		</td>
		<td>
			<?php echo $quizzPoint['QuizzPoint']['quizz_id']; ?>
		</td>
		<td class="actions">
			<?php echo $html->link(__('View', true), array('action' => 'view', $quizzPoint['QuizzPoint']['id'])); ?>
			<?php echo $html->link(__('Edit', true), array('action' => 'edit', $quizzPoint['QuizzPoint']['id'])); ?>
			<?php echo $html->link(__('Delete', true), array('action' => 'delete', $quizzPoint['QuizzPoint']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $quizzPoint['QuizzPoint']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
</table>
</div>
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers();?>
	<?php echo $paginator->next(__('next', true).' >>', array(), null, array('class' => 'disabled'));?>
</div>
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('New QuizzPoint', true), array('action' => 'add')); ?></li>
	</ul>
</div>
